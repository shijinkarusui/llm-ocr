...
### 0.8.0 —— Markdown 清洗内置化与发行稳定性增强

- **内置 Markdown 清洗流水线。** 单页 OCR、批量页面写盘和整书合并现在共用项目内清洗器，运行时不再依赖外部知识库脚本。
- **整书输出格式稳定。** 合并结果统一使用固定页块，保护 Markdown/LaTeX/表格等结构，移除旧版页索引和占位内容，并且绝不虚构缺失页面。
- **持久化与打包更稳健。** 清洗失败或结果为空时不会写入脏 Markdown；便携版侧车已包含清洗模块，并通过健康检查与书籍解析接口冒烟验证。
